...
public class save_list_manager : MonoBehaviour
{
    ...
    {
        SetData();
        del_button.onClick.AddListener(() => { delNamedGame(); SetData(); });
        save_button.onClick.AddListener(() => { saveNamedGame(); exit.onClick.Invoke(); });
        load_button.onClick.AddListener(()=>{ loadNamedGame(); exit.onClick.Invoke(); });
    }
